...
namespace rmrt {
	...
	bool DielectricMaterial::Scatter(const rmrt::Ray& ray, const rmrt::HitRecord& record, rmrt::Color& attenuation, rmrt::Ray& scattered) const
	{
		...
		// The next three lines solve for the angles required to know if  ray is inside the material or not; if it's inside, we must reflect the ray, else we refract it
