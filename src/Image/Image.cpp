...
namespace rmrt {
	...
	Image::Image(std::string_view fileName, float aspectRatio, int width)
		...
		m_maxDiffuseRays(50), m_samplesPerPixel(100), m_samplesIter(std::vector<int>{}), m_file(std::ofstream{}), m_scale(1.0f / m_samplesPerPixel), m_buffer(std::array<char, img_helper::BUFF_SIZE>{}), m_tracker(ProgressTracker{}), m_imgMatrix(std::vector<std::vector<std::string>>{})
	{
	...
	void rmrt::Image::ProgressTracker::PrintProgress(int totalScanLines, int totalWidth, int pixelX, int pixelY, const std::chrono::steady_clock::time_point& startTime)
	{
		...
		progressLine = "\rScan-lines Remaining: " + std::to_string(pixelY) + "    "
			+ "Current Pixel: [ " + std::to_string(pixelX + 1) + " / " + std::to_string(totalWidth) + " ]   "
